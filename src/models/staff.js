...
class Staff extends User {
   ...
   async getStaffs() {
      var q = `select concat(names.surname,', ', names.firstname, ' ', names.othername) as name, user.*, accs.state, user.status,   v.variation, cl.name as class  from ${this.tables.user} as user
        
       join ${this.tables.account} as acc
      
       on acc.id = user.type_id and acc.code = '3'
   ...
   async getStudent(params) {
    ...
    params = Buffer.from(params, 'base64').toString()
      ...
      from ${this.tables.user} as user
       ...
       on acc.id = user.type_id and acc.code = 'Student'
