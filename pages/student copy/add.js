...
import { AEdit, ADelete, AAdd, ActionHeading } from '../../src/actionbtn';
...
function Student_Index({ user, students, info,  classes, Parents , countries, auth_token }) {
    let style = useStyles()
...
function addStudent(e){
    e.preventDefault()
    console.log(Parents)
    let filtered = students.filter(e=> e.name.toLowerCase().includes(`${formInput.surname}, ${formInput.firstname} ${formInput.othername}`.toString().toLowerCase()) )
   ...
   if(filtered.length > 0){
    setOthers(r=>({...r, matched:filtered,match:true}))
    return false
   }
    ...
    return (
        <Dashboard  {...user} page_title="Add Student" crumb={[{ title: 'Students', to: '/student' }]} >
            <form ref={form} method='post' action='/api/student/add'  onSubmit={addStudent}    style={{ display: 'flex', justifyContent: 'center', flexDirection: 'column' }} encType='multipart/form-data' >
